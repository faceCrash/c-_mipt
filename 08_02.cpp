...
public:
  ...
  friend auto &operator<<(std::ostream &stream, Integer const &integer) {
    if (integer.m_is_negative &&
        !(integer.m_size == 1 && integer.m_digits[0] == 0)) {
      stream << '-';
    }
...
int main() {
  ...
  // Modulo
  Integer rem_x = Integer("100") % Integer("30");
  assert(rem_x == Integer("10"));

  rem_x %= Integer("7");
  assert(rem_x == Integer("3"));

  // Pow
  Integer base("2");
  Integer power = pow(base, 10);
  assert(power == Integer("1024"));
