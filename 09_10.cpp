...
public:
  virtual ~Allocator() {
    if (m_begin)
      ::operator delete(m_begin, m_total_size, std::align_val_t(s_alignment));
  }

  virtual void *allocate(std::size_t size,
                         std::size_t alignment = s_alignment) = 0;
...
protected:
  ...
  template <typename T> T *get(void *x) const { return static_cast<T *>(x); }

  std::byte *get_byte(void *x) const { return static_cast<std::byte *>(x); }
...
public:
  ...
  void *allocate(std::size_t size,
                 std::size_t alignment = s_alignment) override {
...
public:
  ...
  void *allocate(std::size_t size,
    ...
    if (void *ptr = std::align(alignment, size, start, space)) {
      std::uint8_t padding = static_cast<std::uint8_t>(
          get_byte(ptr) - (get_byte(m_begin) + m_offset));
      auto header =
          reinterpret_cast<std::uint8_t *>(get_byte(ptr) - header_size);
      *header = padding;
  ...
  void deallocate(void *ptr) override {
    auto header =
        reinterpret_cast<std::uint8_t *>(get_byte(ptr) - sizeof(std::uint8_t));
