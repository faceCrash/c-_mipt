...
int main() {
  ...
  auto filter_drop = nums | std::views::filter([](int n) { return n > 2; }) |
                     std::views::drop(2);
  std::cout << "   Filter (>2) -> Drop(2): ";
  for (int x : filter_drop)
    std::cout << x << " ";
  std::cout << "\n";
  ...
  std::cout << "\n";
  auto strided =
      nums | std::views::filter([i = 0](int) mutable { return i++ % 2 == 0; });
  std::cout << "   Stride (step 2): ";
  for (int x : strided)
    std::cout << x << " ";
  std::cout << "\n\n";
